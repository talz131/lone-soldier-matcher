    ...
    <div>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          <select value={data.referenceCountryCode} onChange={e => onChange({ referenceCountryCode: e.target.value })}
            className="border border-[#d4c9b8] rounded-xl px-2 py-2.5 bg-white text-[#0B2818] text-sm focus:outline-none focus:ring-2 focus:ring-[#534AB7] focus:border-transparent transition shrink-0">
            {COUNTRY_CODES.map(c => <option key={c.code} value={c.code}>{c.label}</option>)}
          </select>
          <input type="tel" value={data.referencePhone} onChange={e => { onChange({ referencePhone: e.target.value.replace(/\D/g, '') }); setErrors(prev => ({ ...prev, referencePhone: '' })) }} className={`flex-1 ${inp(errors.referencePhone)}`} />
        </div>
