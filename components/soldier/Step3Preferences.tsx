...
export default function Step3Preferences({ data, onChange, onNext, onBack }: Props) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-800 mb-1">Family preferences</h2>
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Preferred family observance level
        </label>
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Family vibe <span className="text-gray-400 font-normal">(choose all that appeal to you)</span>
        </label>
        ...
        <div className="flex gap-3">
          {[
            ...
            <button
              ...
              type="button"
              onClick={() => onChange({ petsOk: opt.value })}
      ...
      <div className="mb-8">
        ...
        <div className="flex gap-3 mb-3">
          {[
            ...
            <button
              key={String(opt.value)}
              type="button"
              onClick={() => onChange({ hasDietaryRestrictions: opt.value })}
              className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition-all ${
        ...
        {data.hasDietaryRestrictions && (
          <textarea
            ...
            onChange={e => onChange({ dietaryDetails: e.target.value })}
            className={inp}
            rows={2}
            placeholder="e.g. kosher, vegetarian, allergic to nuts..."
          />
        )}
      </div>
